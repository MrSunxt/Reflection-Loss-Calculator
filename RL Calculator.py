...
class RlCalculate(QThread):
    ...
    def sort_data(self):
        ...
        if path.split('.')[-1] == 'xlsx':
            self.display_sig.emit('正在读取Excel的数据')
            ...
            i_u = []
            if path.split('.')[-1] == 'dat':
                self.display_sig.emit('正在读取 思仪-3672C-S 的数据')
                for i in data[27:]:
                ...
                if data[1] == '\n':
                    self.display_sig.emit('正在读取 安捷伦-E5071C 的数据')
                    ...
                    self.display_sig.emit('正在读取 安捷伦-PNA-N5244A 的数据')
                    for i in data[2:]:
                        f.append(float(i.split()[0]))
                        r_e.append(float(i.split()[1]))
                        i_e.append(float(i.split()[2]))
    ...
    def run(self):
        ...
        item = ['frequency']
        for i in range(101):
            item.append(str(round(i * (thick / 100), 2)) + 'mm')
...
class MyWindow(QWidget):
    ...
    def init_ui(self):
        ...
        self.text.setFixedSize(350, 105)
        self.text.setStyleSheet('background-color:rgb(253, 245, 230)')
        self.text.setPlaceholderText('仪器型号：\n'
                                     '# 思仪 3672C-S (*.dat)\n'
                                     '# 安捷伦 E5071C (*.prn)\n'
                                     '# 安捷伦 PNA-N5244A (*.prn)')
        # 计算类样式
        ...
        # 展示类样式
        self.display_text.setFixedSize(350, 200)
