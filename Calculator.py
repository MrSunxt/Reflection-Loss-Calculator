...
class RlCalculate(QThread):
    ...
    def merge_data(self):
        ...
        for i in range(0, len(data[0])):
            e.append(complex(data[1][i], -data[2][i]))
            u.append(complex(data[3][i], -data[4][i]))
        return e, u
    ...
    def run(self):
        ...
        ws_im.append(
            [str(round(i * (thick / 100), 2)) + 'mm' for i in range(101)])
        self.display_sig.emit('正在计算中，厚度：{}mm'.format(thick))
        for i in range(0, len(f)):
            list_rl = []
            list_im = []
            for j in range(0, 101):
                d = round((thick/100000) * j, 5)
                zin = cmath.sqrt(u[i] / e[i]) * cmath.tanh(complex(0, 1) * ((2 * cmath.pi * f[i] * 1000000000 * d) / 300000000) * cmath.sqrt(u[i] * e[i]))
                im = abs(zin)
        ...
        wb.close()
        t2 = self.check_time()
        self.display_sig.emit('结束，耗时：{:.4f}s'.format(t2 - t1))
...
class AlphaCalculate(QThread):
    ...
    def run(self):
        ...
        for i in range(0, len(f)):
            list_alpha = []
            part1 = (math.sqrt(2) * math.pi * f[i] * 1000000000) / 300000000
        ...
        wb.close()
        t4 = self.check_time()
        self.display_sig.emit('结束，耗时：{:.4f}s'.format(t4 - t3))
...
class MyWindow(QWidget):
    ...
    def init_ui(self):
        ...
        self.c_button2.setFixedSize(100, 50)
        ...
        # 展示类样式
        self.display_text.setFixedSize(350, 110)
    ...
    def click_about(self):
        msg = 'Version：1.2.4<br>' \
              'Designer：Xuetao Sun<br>Download：' \
              '<a href="https://github.com/MrSunxt/Reflection-Loss-Calculator/releases">GitHub</a>'
        QMessageBox.about(self, 'About', msg)

    def click_rl(self):
        if path == '':
            self.display_text.setText('{:^40}\n{:^40}\n{:^40}'
                                      .format('！！！ 请先选择文件 ！！！',
                                              '！！！ 请先选择文件 ！！！',
                                              '！！！ 请先选择文件 ！！！'))
        else:
